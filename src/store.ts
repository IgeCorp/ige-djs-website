...
export const store = createStore<State>({
	state: {
		...
		file: null,
		stats: {
			downloads: `${(258).toLocaleString()}+`,
			stars: `${(3).toLocaleString()}+`,
	...
	actions: {
		fetchStats: async ({ commit }) => {
			...
			const [fetchedDownloads, fetchedStars, fetchedContributors] = await Promise.all([
				fetch('https://api.npmjs.org/downloads/range/2013-08-21:2100-08-21/@igecorp/ige-djs').then(toJSON, noop),
				fetch('https://api.github.com/repos/IgeCorp/ige-djs').then(toJSON, noop),
				fetch('https://api.github.com/repos/IgeCorp/ige-djs/stats/contributors').then(toJSON, noop),
			]);
